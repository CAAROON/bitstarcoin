...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bitstarcoin"))
        return false;
...
{
    // Convert bitstarcoin:// to bitstarcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitstarcoin://"))
    {
        uri.replace(0, 12, "bitstarcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitstarCoin.lnk";
}
...
{
    return GetAutostartDir() / "bitstarcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitstarCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitstarcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BitstarCoin-Qt"));
